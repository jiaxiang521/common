...
/*
* 手机滚动函数
* @wrapWidth   宽度固定的包裹层宽度
* @wrapAllWidth		真实包裹层宽度
* @wrap		真实包裹层对象
*/
// 手机滚动函数
function mob_scroll(wrapWidth, wrapAllWidth, wrap){
	var iScroll=0;
	var iStartX=0;
	var iStartPageX=0;
	var iNow=0;
	...
	wrap.ontouchstart=function(ev)
	{
		...
		iStartX=iScroll;
		wrap.style.WebkitTransition=wrap.style.MozTransition=wrap.style.transition="none";
	};
	wrap.ontouchmove=function(ev)
	{
		var iDis=ev.changedTouches[0].pageX-iStartPageX;
		iScroll=iStartX+iDis;
		document.title = iScroll;
		if(iScroll > 0){
			iScroll = 0;
		}
		if( iScroll < -(wrapAllWidth-wrapWidth) ){
			iScroll = -(wrapAllWidth-wrapWidth);
		}
		setStyle();
	};
	function setStyle()
	{
		wrap.style.WebkitTransform=wrap.style.MozTransform=wrap.style.transform="translateX("+iScroll+"px)";
	}
}
...
function getRect(elem){
	...
	if(elem.getBoundingClientRect){
		...
		var actualLeft = getAbPos(elem).left,
			actualTop = getAbPos(elem).top;
		return {
			left : actualLeft - scrollLeft,
			right : actualLeft - scrollLeft + elem.offsetWidth,
			top : actualtop - scrolltop,
			bottom : actualtop - scrolltop + elem.offsetHeight
		};
	}
}
